...
st.markdown("""
    ...
    .big-header {background-color: #334155; color: white; padding: 30px; border-radius: 10px; font-size: 42px; font-weight: 800; text-align: center; margin-bottom: 25px;}
    .section-header {background-color: #ccfbf1; color: #0f172a; padding: 12px 20px; border-radius: 8px; font-size: 22px; font-weight: 700; margin-top: 25px; margin-bottom: 15px; border-left: 8px solid #2dd4bf;}
    .mission-box {background-color: #f8fafc; padding: 25px; border-radius: 15px; border: 1px solid #e2e8f0; margin-bottom: 25px;}
    </style>
    """, unsafe_allow_html=True)

# Fixed NGO Palette for consistent legends
...
def load_and_prep_data():
    ...
    # Encounter Year (Year of the actual visit)
...
try:
    ...
    if page == "Overview & Feedback":
        ...
        st.markdown('<div class="mission-box">To dismantle <strong>Vertical Equity Gaps</strong> by providing transparent data on healthcare costs for underserved populations.</div>', unsafe_allow_html=True)
        ...
        st.info("💡 **Summary:** Hover over counties to see Average Income and Insurance Coverage %.")
        ...
        # TILE CHOROPLETH (No Lat/Lon shown, only pure stats)
        fig = px.choropleth(
            map_stats,
        ...
        st.markdown('<div class="section-header">Deep-Dive: County Analytics</div>', unsafe_allow_html=True)
        sel_county = st.selectbox("Select County:", sorted(df['COUNTY'].unique()))
        c_df = df[df['COUNTY'] == sel_county]
        
        c_col1, c_col2 = st.columns(2)
        with c_col1:
            x_axis = st.selectbox("Choose X-Axis (Indep. Variable):", ['GENDER', 'RACE', 'INCOME_TIER'], key="x1")
            y_axis = st.selectbox("Choose Y-Axis (Dep. Variable):", ['TOTAL_CLAIM_COST', 'INCOME', 'INSURANCE_COVERAGE_PCT'], key="y1")
            st.plotly_chart(px.bar(c_df.groupby(x_axis)[y_axis].mean().reset_index(), x=x_axis, y=y_axis, color=x_axis, color_discrete_sequence=NGO_PALETTE), use_container_width=True)
        
        with c_col2:
            st.write(f"**Quick Statistics for {sel_county}**")
        ...
        with col_a:
            demo_a = st.selectbox("Compare Group A by:", ['RACE', 'GENDER', 'INCOME_TIER'], key="comp_a")
            # Forcing legend consistency using NGO_PALETTE
            fig_a = px.bar(df.groupby(demo_a)[metric].mean().reset_index(), x=demo_a, y=metric, color=demo_a, color_discrete_sequence=NGO_PALETTE)
            st.plotly_chart(fig_a, use_container_width=True)
            
        with col_b:
            demo_b = st.selectbox("Compare Group B by:", ['RACE', 'GENDER', 'INCOME_TIER'], key="comp_b")
            fig_b = px.bar(df.groupby(demo_b)[metric].mean().reset_index(), x=demo_b, y=metric, color=demo_b, color_discrete_sequence=NGO_PALETTE)
        ...
        st.plotly_chart(px.line(combined, x='YEAR', y=target, color='Status', markers=True, color_discrete_map={'Actual':'#94a3b8', 'Projected':'#fb7185'}), use_container_width=True)

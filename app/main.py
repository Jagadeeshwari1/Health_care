...
try:
    ...
    if page == "Overview":
        ...
        with st.form("feedback"):
            ...
            st.text_input("Name"); st.text_area("Message")
        ...
        st.markdown('<div class="section-header">Regional Expenditure Analysis</div>', unsafe_allow_html=True)
        ...
        # FALLBACK LOGIC: Try Choropleth, but if it's blank/fails, show Scatter Map
        try:
            # We attempt the Tile Choropleth
            fig = px.choropleth(
                map_stats, geojson="https://raw.githubusercontent.com/codeforgermany/click_dummy/master/public/data/ca-counties.json",
                locations='COUNTY', featureidkey="properties.NAME",
                color='TOTAL_CLAIM_COST', color_continuous_scale="Teal", scope="usa",
                hover_data={'COUNTY': True, 'TOTAL_CLAIM_COST': ':,.2f', 'INCOME': ':,.0f', 'INSURANCE_COVERAGE_PCT': ':.1f%'}
            )
            fig.update_geos(fitbounds="locations", visible=False)
            st.plotly_chart(fig, use_container_width=True, key="tile_map")
        except:
            # If Tile Map fails, show Bubble Map (uses internal Lat/Lon data)
            st.warning("🔄 Switching to high-reliability coordinate map...")
            fig_bub = px.scatter_mapbox(
                map_stats, lat="LAT", lon="LON", color="TOTAL_CLAIM_COST", size="TOTAL_CLAIM_COST",
                hover_name="COUNTY", color_continuous_scale="Teal", size_max=20, zoom=5, mapbox_style="carto-positron"
            )
